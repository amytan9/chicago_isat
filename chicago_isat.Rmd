...
x <- x %>% mutate(hhi = ((percent_white/100)^2 + (percent_afam/100)^2 + (percent_asian/100)^2 + (percent_hisp/100)^2 + (percent_natam/100)^2 + (percent_multi/100)^2 + (percent_hawpac/100)^2))
...
plot_hhi_bell <- ggplot(data=subset(x, school== "Bell"), aes(year, hhi, group = se_elem, color = se_elem)) +
  stat_summary(geom = 'line') +
   scale_x_continuous(breaks = 0:2100) +
    geom_vline(xintercept = 2011) +
    theme_minimal()

plot_test_bell <- ggplot(data=subset(x, school== "Bell"), aes(year, new_composite_12, group = se_elem, color = se_elem)) +
  stat_summary(geom = 'line') +
   scale_x_continuous(breaks = 0:2100) +
    geom_vline(xintercept = 2011) +
    theme_minimal()
...
master <- merge(x=x,y=iep_comb,by=c("school_id", "year"),all=TRUE)
