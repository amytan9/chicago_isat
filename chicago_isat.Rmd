...
# reading in demographic data by year
...
score_data <- y %>% filter(year != 2001) %>% filter(year != 2002) %>% filter(year != 2003) %>% filter(year != 2004) %>% filter(year != 2005) 
# no ISAT score data for 2015 so filter from other datasets as well for master_2 dataset

master <- master %>% filter(year != 2015)

master_2 <- merge(x=master, y=score_data, by=c("school_id", "year"), all=TRUE)
```
...
slope_test <- lm(percent_white ~ se_elem.x*year, data = filter(master_2, policy_year == 0))
slope_test1 <- lm(percent_afam ~ se_elem.x*year, data = filter(master_2, policy_year == 0))
slope_test2 <- lm(percent_natam ~ se_elem.x*year, data = filter(master_2, policy_year == 0))
slope_test3 <- lm(percent_asian ~ se_elem.x*year, data = filter(master_2, policy_year == 0))
slope_test4 <- lm(percent_hisp ~ se_elem.x*year, data = filter(master_2, policy_year == 0))

# should probably put this later since HHI is coded in a later chunk
slope_test5 <- lm(hhi ~ se_elem.x*year, data = filter(master_2, policy_year == 0))

# slope test for test scores
slope_test6 <- lm(new_composite_16 ~ se_elem.x*year, data = filter(master_2, policy_year == 0))
...
stargazer(slope_test, slope_test1, slope_test2, slope_test3, 
          slope_test4, slope_test5, slope_test6,
          type="html", 
          dep.var.labels.include = FALSE,
          column.labels = c("% White", "% African American", "% Native American", "% Asian", "% Hispanic", "HHI", "ISAT Scores"),
          covariate.labels = c("Selective Enrollment School", "Post Policy Change", "Selective Enrollment*Policy Change"),
          out="slope_test_by_race_1.html")
```
...
```
```{r}
# cow plot to put all these together
race_plots_comb <- plot_grid(plot1, 
```
