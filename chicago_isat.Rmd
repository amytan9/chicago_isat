...
examplereg = lm(new_composite_12 ~ se_elem + policy_year + did + percent_bil, data = x)
...
for (i in seq_along(se_school_ids)) { # Loop over loop.vector
   ...
   plot <- 
      ggplot(subset(df, df$school_id==se_school_ids[i]),
             aes(year, hhi, group = se_elem, colour = se_elem)) + 
      ...
      ggtitle(paste(se_school_ids[i], ' County, New Jersey \n', 
