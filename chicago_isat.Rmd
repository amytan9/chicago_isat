...
# unlisted the composite_16 variable in order to make it a numeric variable 
...
score_plot <- ggplot(y, aes(year, new_composite_16, group = se, color = se)) +
...
# add datasets vertically as a test
# MAYBE TAKE THIS CHUNK OUT, replicated later with test1

test <-bind_rows(demo_2006, demo_2007, demo_2008, demo_2009, demo_2010, demo_2011, demo_2012, demo_2013, demo_2014, demo_2015)
...
# started at 2006 to give five years of demographic data before the change was implemented in the 2011-ending year
...
# don't use this for now, check to see if new hhi is ok

x <- x %>% mutate_all(~replace(., is.na(.), 0))
...
<!-- ```{r} -->
<!-- # old method of filtering for SE schools -->
...
x_with_hhi <- x %>% mutate(hhi = ((percent_white/100)^2 + (percent_afam/100)^2 + (percent_api/100)^2 + (percent_hisp/100)^2 + (percent_natam/100)^2 + (percent_multi/100)^2 ))

reg_hhi = lm(hhi ~ se_elem + policy_year + did, data = x_with_hhi)

summary(reg_hhi)

# overall plot of HHI for SE v. non-SE schools

plot_hhi <- ggplot(x_with_hhi, aes(year, hhi, group = se_elem, color = se_elem)) +
  stat_summary(geom = 'line') +
   scale_x_continuous(breaks = 0:2100) +
    geom_vline(xintercept = 2011) +
    theme_minimal()

# plot of a specific school: (prob don't need with my function below)

plot_hhi_bell <- ggplot(data=subset(x_with_hhi, school== "Bell"), aes(year, hhi, group = se_elem, color = se_elem)) +
  stat_summary(geom = 'line') +
   scale_x_continuous(breaks = 0:2100) +
    geom_vline(xintercept = 2011) +
    theme_minimal()

plot_test_bell <- ggplot(data=subset(x_with_hhi, school== "Bell"), aes(year, new_composite_12, group = se_elem, color = se_elem)) +
  stat_summary(geom = 'line') +
   scale_x_continuous(breaks = 0:2100) +
    geom_vline(xintercept = 2011) +
    theme_minimal()
...
master <- merge(x=x_with_hhi, y=iep_comb, by=c("school_id", "year"), all=TRUE)

# making sure it's the right amount of SE school entries

master %>% filter(school_id %in% missing_school_ids) %>% glimpse()

# merging with ISAT score data 

y_subset <- y %>% filter(year != 2001) %>% filter(year != 2002) %>% filter(year != 2003) %>% filter(year != 2004) %>% filter(year != 2005) 

# no ISAT score data for 2015 so filter from other datasets as well for master_2 dataset

master_subset <- master %>% filter(year != 2015)

master_2 <- merge(x=master_subset, y=y_subset, by=c("school_id", "year"), all=TRUE)
...
## should I add hhi as a variable here?- ASK (yes)

score_reg = lm(new_composite_16 ~ se_elem.x + policy_year + did.x + percent_bil, data = master_2)

score_reg_2 = lm(new_composite_16 ~ se_elem.x + policy_year + did.x + hhi + percent_bil, data = master_2)
...
# making a vertical dataset with variables of interest
hhi.long <- melt(x_with_hhi, id.vars=c("year", "hhi", "school_id"))
...
test.long <- melt(y_edit, id.vars=c("year", "new_composite_16", "school_id", "grade"))
...
test.graph <- function(df, na.rm = TRUE, ...){
  ...
  for (i in seq_along(se_school_ids)) { # Loop over loop.vector
   ...
   test_p <- 
      ggplot(subset(df2, df2$school_id==se_school_ids[i]),
             aes(year, new_composite_16, group = school_id)) + 
      geom_line(size=2) +
      facet_wrap( ~  school_id, ncol=3) +
      geom_vline(xintercept = 2011) +
      theme_pander() +
      theme(legend.position="none") + 
